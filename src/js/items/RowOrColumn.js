...
lm.utils.copy(lm.items.RowOrColumn.prototype, {
  ...
  removeChild: function (contentItem, keepChild) {
    ...
    var splitterIndex = Math.max(index - 1, 0)
    ...
    if (this.contentItems.length === 1 && this.config.isClosable === true) {
      var childItem = this.contentItems[ 0 ]
      this.contentItems = []
  ...
  dock: function (contentItem, mode, collapsed) {
    ...
    var removedItemSize = contentItem.config[ this._dimension ]
    var headerSize = this.layoutManager.config.dimensions.headerHeight
    var index = lm.utils.indexOf(contentItem, this.contentItems)
    var splitterIndex = Math.max(index - 1, 0)
    var i
    ...
    if (isDocked) { // undock it
      ...
      for (i = 0; i < this.contentItems.length; i++) {
        ...
        if (this.contentItems[ i ] === contentItem) {
          ...
          var itemSize = this.contentItems[ i ].config[ this._dimension ] *= (100 - newItemSize) / 100
          this.contentItems[ i ].config[ this._dimension ] = itemSize
        }
      }
  ...
  _$init: function () {
    ...
    for (var i = 0; i < this.contentItems.length - 1; i++) {
  ...
  _onSplitterDragStop: function (splitter) {
    var items = this._getItemsForSplitter(splitter)
    var sizeBefore = items.before.element[ this._dimension ]()
    var sizeAfter = items.after.element[ this._dimension ]()
    var splitterPositionInRange = (this._splitterPosition + sizeBefore) / (sizeBefore + sizeAfter)
    var totalRelativeSize = items.before.config[ this._dimension ] + items.after.config[ this._dimension ]
