...
lm.utils.copy(lm.items.Stack.prototype, {
  ...
  dock: function (mode) {
    if (this._header.dock) {
      if (this.parent instanceof lm.items.RowOrColumn) { this.parent.dock(this, mode) }
    }
  },
  setSize: function () {
    if (!this.element.is(':visible')) return
    var isDocked = this._docker && this._docker.docked
    var content = { width: this.element.width(), height: this.element.height() }
  ...
  addChild: function (contentItem, index) {
    ...
    this._$validateClosability()
    if (this.parent instanceof lm.items.RowOrColumn) { this.parent._validateDocking() }
    this.emitBubblingEvent('stateChanged')
  },
  ...
  _$getArea: function () {
    ...
    var headerArea = getArea.call(this, this.header.element)
    var contentArea = getArea.call(this, this.childElementContainer)
    var contentWidth = contentArea.x2 - contentArea.x1
    var contentHeight = contentArea.y2 - contentArea.y
