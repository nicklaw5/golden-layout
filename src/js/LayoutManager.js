...
lm.utils.copy(lm.LayoutManager.prototype, {
  ...
  createPopout: function (configOrContentItem, dimensions, parentId, indexInParent) {
    ...
    var isItem = configOrContentItem instanceof lm.items.AbstractContentItem
    var self = this
    var windowLeft
    var windowTop
    var offset
  ...
  _createConfig: function (config) {
    ...
    if (windowConfigKey) {
      ...
      config = window.localStorage.getItem(windowConfigKey)
      config = JSON.parse(config)
      config = (new lm.utils.ConfigMinifier()).unminifyConfig(config)
      window.localStorage.removeItem(windowConfigKey)
    }
  ...
  _adjustToWindowMode: function () {
     ...
     */
    var x = document.body.offsetHeight // eslint-disable-line
